...
class SimpleValidation(ValidationStrategy):
    ...
    def validate(self, X, y, trainIndices, testIndices, description="ExperimentName", threshold=None):
        """
        ...
        #Dan edit
        ytrain = [y[t] for t in trainIndices]
        ...
        for i in testIndices:
            #print "Predicting %s/%s." % (count, len(testIndices))
            prediction = self.model.predict(X[i])
            #print 'comparing %s to %s' % (prediction, y[i])
            print prediction[1]
            predicted_label = prediction[1]['labels'][0]

            identified = True

            if threshold:
                if prediction[1]['distances'][0] > threshold:
                    identified = False
